...
function operarIntruso() {
    ...
    resultado.textContent = `La colecta de reunio un total de: ${totalRecaudado}`
    container.classList.remove(`d-none`)
}
____________________________________________________________
...
function invitados() {
    ...
    for (i = 0; i < temperaturas.length; i++) {
        if (temperaturas[i] >= 30 &&temperaturas[i] < 50) {
            result8.innerHTML +=
                `<p class="text-danger">¡ALERTA! Temperatura crítica de ${temperaturas[i]} grados.</p><br>`;
